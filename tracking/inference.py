...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        prisonPosition=self.getJailPosition()

        allPossible = util.Counter()
        
        for p in self.legalPositions:
            #check if the ghost is in the prison
            if noisyDistance==None:
                allPossible[prisonPosition]=1.0
                allPossible[p]=0.0
            else:
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        pacmanPosition = gameState.getPacmanPosition()
        oldBelief=util.Counter()
        oldBelief=self.beliefs
        newBelief=util.Counter()
        for oldPos in oldBelief:
#            print "oldPos", oldPos
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
#            print "newPosDist", newPosDist
#            print "oldBelief", oldBelief           
            for newPos, prob in newPosDist.items():
#                print "newPos", newPos
                newBelief[newPos]+=oldBelief[oldPos]*prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.initialParticleList=listOfParticles
    ...
    def observe(self, observation, gameState):
        ...
        if allPossible.totalCount()==0:
            ...
            for i in range(0, numParticles):
                anotherList.append(util.sample(allPossible))
            self.initialParticleList=anotherList
    ...
    def elapseTime(self, gameState):
        ...
        anotherList=[]
        for i in range(0, numParticles):
            anotherList.append(util.sample(newBelief))
        self.initialParticleList=anotherList
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        particleList=self.initialParticleList
        beliefDist=util.Counter()
        number=len(particleList)
        for particle in particleList:
            if beliefDist[particle]==None:
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        iterLegalPos=itertools.product(legalPos,legalPos)
        legalMulPos=list(iterLegalPos)

        random.shuffle(legalMulPos)

        for x in range(0, self.numParticles):
            if (x+1)%len(legalMulPos)==0:
                for pos in legalMulPos:
                    listOfParticles.append(pos)
        remainder=numParticles%len(legalPos)
        for i in range(0, remainder):
            if i<len(legalPos):
                listOfParticles.append(legalMulPos[i])
            else:
                listOfParticles.append(legalMulPos[0])
        self.particleList=listOfParticles

        #print "self.particleList", self.particleList
    ...
    def observeState(self, gameState):
        ...
        inBeliefs=util.Counter()
        inBeliefs=self.getBeliefDistribution()
        for p in self.particleList:
            ...
            for i in range(self.numGhosts):
                ...
                if noisyDistances[i]==None:
                    ...
                    cumulativeEmission=cumulativeEmission*emissionModel[trueDistance]
            nextBelief[p]+=cumulativeEmission
                    
                
        nextBelief.normalize()
        
        if nextBelief.totalCount()==0:
            self.initializeParticles()
        else:
            anotherList=[]
            for i in range(0, self.numParticles):
                anotherList.append(util.sample(nextBelief))
            self.particleList=anotherList
    ...
    def elapseTime(self, gameState):
        ...
        """
        newParticles = []
        #print "self.particelist", self.particleList
        for oldParticle in self.particleList:
            ...
            for i in range(self.numGhosts):
                ...
                #print "after sample newParticle", newParticle
